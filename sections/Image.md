...
Those hashes map directly to the `<sha>/layer.tar` files that were extracted when we exploded the image earlier. This is how docker understand which layers to use to build the image for the container.
